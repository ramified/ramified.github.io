(function(root, factory) {
  ...
  const CONTROLLERS = Object.freeze({ HUMAN: 'human', LOCAL_AI: 'local-ai-challenging', HIDDEN: 'hidden' });
  ...
  function chooseMove(state, options = {}) {
    ...
    const budgets = state.gameMode === 'connect-four' ? [150, 240] : (state.gameMode === 'chinese-checkers' ? [220, 320] : [180, 280]);
    const context = {
      startedAt,
    ...
    if (state.gameMode === 'gomoku') return chooseGomoku(state, topology, rules, context);
  ...
  return {
    ...
    __test: {
      ...
      clearCaches() { topologyCache.clear(); transpositions.clear(); },
      cacheSizes() { return { topology: topologyCache.size, transpositions: transpositions.size }; }
    }
  };
});
