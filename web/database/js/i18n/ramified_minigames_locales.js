(() => {
  ...
  const pairs = {
    ...
    'ai.players': ['Players', '玩家'],
    'ai.human': ['Human', '人类'],
    'ai.local': ['Local AI', '本地 AI'],
    'ai.localChallenging': ['Local AI — Challenging', '本地 AI — 挑战'],
    'ai.localAggressive': ['Local AI — Aggressive', '本地 AI — 激进型'],
    'ai.hidden': ['Not shown', '不显示'],
    'ai.pause': ['Pause AI', '暂停 AI'],
    'ai.resume': ['Resume AI', '继续 AI'],
    'ai.thinking': ['Local AI is thinking…', '本地 AI 正在思考…'],
    'ai.paused': ['Local AI paused', '本地 AI 已暂停'],
    'ai.unavailable': ['Local AI unavailable', '本地 AI 不可用'],
    'ai.moveInfo': ['{{side}} is choosing a move locally', '{{side}}正在本地选择一步棋'],
    'ai.errorInfo': ['The AI was paused; the current game is unchanged.', 'AI 已暂停；当前棋局没有改变。'],
    'ai.playerRequired': ['Keep at least one Chinese Checkers color visible.', '请至少保留一种可见的跳棋颜色。'],
    'ai.showFivePathCounts': ['show five-path counts', '显示成五路径数'],
    'ai.challengingThinkTime': ['Challenging AI think time', '挑战型 AI 思考时间'],
    'ai.safePruning': ['pruning safety fallback', '剪枝安全兜底'],
    'ai.undoExchange': ['restored before the human and Local AI exchange', '已恢复到人类与本地 AI 交手之前'],
    'runtime.checkersMoving': ['{{side}} is moving', '{{side}}正在移动'],
    'ai.sightStyle': ['AI sight', 'AI 瞄准标记'],
    'ai.sightCrosshair': ['Classic crosshair', '经典准星'],
    'ai.sightCorners': ['Corner brackets', '四角框'],
    'ai.sightRings': ['Concentric target', '同心靶环'],
    'ai.sightDuration': ['Sight duration', '标记时长'],
    'ai.sightPreview': ['preview', '预览'],
    'ai.access.blackController': ['Black controller', '黑方控制者'],
    'ai.access.whiteController': ['White controller', '白方控制者'],
    'ai.access.redController': ['Red controller', '红方控制者'],
    'ai.access.yellowController': ['Yellow controller', '黄方控制者'],
    'ai.access.colorController': ['{{side}} controller', '{{side}}控制者'],
    'ai.access.checkersControllers': ['Chinese Checkers controllers', '中国跳棋控制者'],
    'ai.access.showFivePathCounts': ['Show Gomoku five-path counts', '显示五子棋成五路径数'],
    'ai.access.challengingThinkTime': ['Gomoku Challenging AI think time', '五子棋挑战型 AI 思考时间'],
    'ai.access.safePruning': ['Use Gomoku pruning safety fallback', '使用五子棋剪枝安全兜底'],
    'ai.access.sightStyle': ['Chinese Checkers AI sight style', '中国跳棋 AI 瞄准标记样式'],
    'ai.access.sightDuration': ['Chinese Checkers AI sight duration', '中国跳棋 AI 瞄准标记时长'],
    'common.canvasSpark': ['canvas spark', '画布火花'],
    'debug.checkTranslation': ['check translation', '检查翻译'],
    'debug.soundVariants': ['Sound effect variants', '音效变体'],
    'debug.soundAudition': ['Seeded retro sound audition', '种子复古音效试听'],
    'sfx.variant.physical': ['Physical', '拟真'],
    'sfx.variant.clear': ['Clear', '清晰'],
    'sfx.variant.arcade': ['Arcade', '街机'],
    'sfx.preview': ['Preview', '试听'],
    'sfx.audition.category': ['Collection', '音色集合'],
    'sfx.audition.applyTo': ['Apply to event', '应用到事件'],
    'sfx.audition.save': ['Save for event', '保存到事件'],
    'sfx.audition.generate': ['Generate 24', '生成 24 个'],
    'sfx.audition.customSeed': ['Numeric seed', '数字种子'],
    'sfx.audition.useSeed': ['Use seed', '使用种子'],
    'sfx.audition.seedValue': ['Seed {{seed}}', '种子 {{seed}}'],
    'sfx.audition.marble': ['Marble hops', '弹珠跳跃'],
    'sfx.audition.stone': ['Stone skips', '掠水石'],
    'sfx.audition.billiards': ['Billiards contacts', '台球碰撞'],
    'sfx.audition.win': ['Wins', '获胜'],
    'sfx.audition.marbleHopOne': ['Marble hop 1', '弹珠跳跃 1'],
    'sfx.audition.marbleHopTwo': ['Marble hop 2', '弹珠跳跃 2'],
    'sfx.audition.marbleHopThree': ['Marble hop 3', '弹珠跳跃 3'],
    'sfx.audition.stoneSkipOne': ['Stone skip 1', '掠水石 1'],
    'sfx.audition.stoneSkipTwo': ['Stone skip 2', '掠水石 2'],
    'sfx.audition.stoneSkipThree': ['Stone skip 3', '掠水石 3'],
    'sfx.audition.billiardsOne': ['Billiards contact 1', '台球碰撞 1'],
    'sfx.audition.billiardsTwo': ['Billiards contact 2', '台球碰撞 2'],
    'sfx.audition.billiardsThree': ['Billiards contact 3', '台球碰撞 3'],
    'sfx.audition.winOne': ['Win 1', '获胜 1'],
    'sfx.audition.winTwo': ['Win 2', '获胜 2'],
    'sfx.audition.winThree': ['Win 3', '获胜 3'],
    'sfx.event.action': ['Accepted action', '成功操作'],
    'sfx.event.invalid': ['Invalid action', '无效操作'],
    'sfx.event.win': ['Win', '获胜'],
    'sfx.event.draw': ['Draw', '和局'],
    'sfx.event.restart': ['Restart', '重新开始'],
    'sfx.event.undo': ['Undo', '撤销'],
    'sfx.event.redo': ['Redo', '重做'],
    'sfx.event.slide': ['Slide', '滑动'],
    'sfx.event.merge': ['Merge', '合并'],
    'sfx.event.bomb': ['Bomb', '炸弹'],
    'sfx.event.gameover': ['Game over', '游戏结束'],
    'sfx.event.place': ['Place piece', '落子'],
    'sfx.event.capture': ['Capture', '提子'],
    'sfx.event.flip': ['Flip discs', '翻转棋子'],
    'sfx.event.chessMove': ['Chess move', '国际象棋走子'],
    'sfx.event.chessCapture': ['Chess capture', '国际象棋吃子'],
    'sfx.event.checkersMove': ['Checker move', '跳棋移动'],
    'sfx.event.checkersJump': ['Checker jump', '跳棋跳跃'],
    'sfx.event.checkersLakeHop': ['Lake-stone hop', '掠水石跳跃'],
    'sfx.event.checkersGoal': ['Checker goal', '跳棋到达目标'],
    'sfx.event.tileSelect': ['Tile selection', '方块选择'],
    'sfx.event.tileMatch': ['Tile match', '方块配对'],
    'sfx.event.tileMismatch': ['Tile mismatch', '方块不匹配'],
    'sfx.event.tileShuffle': ['Tile shuffle', '方块重排'],
    'sfx.event.tileCleared': ['Board cleared', '棋盘清空'],
    'sfx.event.sokobanStep': ['Sokoban step', '推箱子走步'],
    'sfx.event.sokobanPush': ['Sokoban push', '推箱子推箱'],
    'sfx.event.sokobanBlocked': ['Sokoban blocked', '推箱子受阻'],
    'sfx.event.sokobanTarget': ['Target filled', '目标填满'],
    'sfx.event.sokobanSolved': ['Puzzle solved', '谜题完成'],
    'sfx.event.billiardsStrike': ['Cue strike', '球杆击球'],
    'sfx.event.billiardsBall': ['Ball collision', '球球碰撞'],
    'sfx.event.billiardsCushion': ['Cushion contact', '碰库'],
    'sfx.event.billiardsPocket': ['Pocket', '进袋'],
    'sfx.event.billiardsFoul': ['Billiards foul', '台球犯规'],
    'debug.hexCoverOffset': ['Hex cover offset', '六边形覆盖偏移'],
    'debug.offsetX': ['X', 'X'],
    'debug.offsetY': ['Y', 'Y'],
    'debug.hexCoverOffsetValue': ['{{value}} r', '{{value}} r'],

    'fullscreen.settings': ['Settings', '设置'],
    'fullscreen.wrappedView': ['Board view', '棋盘视图'],
    'fullscreen.soundEffects': ['Sound effects', '音效'],
    'fullscreen.volume': ['Volume', '音量'],
    'fullscreen.showActionRow': ['Show action row', '显示操作栏'],
    'fullscreen.showGameTools': ['Show game tools', '显示游戏工具'],
    'fullscreen.exit': ['Exit fullscreen', '退出全屏'],
    'fullscreen.restart': ['Restart', '重新开始'],
    'fullscreen.confirmRestart': ['Confirm restart', '确认重新开始'],

    'controls.settingsSections': ['Settings sections', '设置分区'],
    'controls.controlsTab': ['Controls & cards', '键位与卡片'],
    'controls.displayTab': ['Display & sound', '显示与声音'],
    'controls.ui.settings': ['Settings', '设置'],
    'controls.ui.openSettings': ['Open settings', '打开设置'],
    'controls.ui.close': ['Close', '关闭'],
    'controls.ui.keyboard': ['Keyboard shortcuts', '键盘快捷键'],
    'controls.ui.pointer': ['Mouse & touch hints', '鼠标与触控提示'],
    'controls.ui.restore': ['Restore defaults', '恢复默认设置'],
    'controls.ui.cards': ['Cards', '卡片'],
    'controls.ui.cardsHelp': ['Choose which cards are shown on this page.', '选择在此页面显示哪些卡片。'],
    'controls.ui.advanced': ['Advanced', '高级'],
    'controls.ui.cardsRestored': ['Default settings restored.', '已恢复默认设置。'],
    'controls.ui.addBinding': ['Add key', '添加键位'],
    'controls.ui.editBinding': ['Change {{binding}}', '修改 {{binding}}'],
    'controls.ui.removeBinding': ['Remove binding', '删除键位'],
    'controls.ui.pressKeys': ['Press a key combination…', '请按下键位组合…'],
    'controls.ui.locked': ['Fixed interaction', '固定操作'],
    'controls.ui.noBindings': ['No key assigned', '未分配键位'],
    'controls.ui.captureCancelled': ['Key capture cancelled.', '已取消键位录入。'],
    'controls.ui.bindingRemoved': ['Binding removed.', '已删除键位。'],
    'controls.ui.bindingSaved': ['Binding saved.', '已保存键位。'],
    'controls.ui.bindingLimit': ['An action can use at most {{count}} keys.', '每项操作最多可使用 {{count}} 个键位。'],
    'controls.ui.reserved': ['That key is reserved by the browser or the settings dialog.', '该键位由浏览器或设置窗口保留。'],
    'controls.ui.duplicate': ['That key is already assigned to this action.', '该键位已分配给此操作。'],
    'controls.ui.conflict': ['{{binding}} is currently assigned to “{{action}}”. Replace it?', '{{binding}} 已用于“{{action}}”。是否替换？'],
    'controls.ui.replace': ['Replace', '替换'],
    'controls.ui.cancel': ['Cancel', '取消'],
    'controls.ui.restored': ['Default key bindings restored.', '已恢复默认键位。'],
    'controls.ui.choosePanel': ['Choose or focus a panel before running its primary action.', '请先选择或聚焦一个面板，再执行其主操作。'],
    'controls.ui.unavailable': ['This action is not available in the current state.', '此操作在当前状态下不可用。'],
    'controls.ui.profile': ['Current profile: {{profile}}', '当前方案：{{profile}}'],
    'controls.ui.keysHelp': ['Click a key to change it. Use Add key to keep multiple alternatives.', '点击键位即可修改；使用“添加键位”可保留多个备选键。'],
    'controls.ui.pointerHelp': ['Mouse and touch gestures are shown for reference and cannot be remapped.', '鼠标与触控手势仅供参考，不能改绑。'],
    'controls.ui.clearBinding': ['Press Backspace or Delete to clear this binding; Escape cancels.', '按 Backspace 或 Delete 删除此键位；按 Escape 取消。'],
    'controls.groupHistory': ['History', '历史'],
    'controls.groupGame': ['Game', '游戏'],
    'controls.groupDirections': ['Directions', '方向'],
    'controls.undo': ['Undo', '撤销'],
    'controls.undoDescription': ['Restores the previous game step.', '恢复到上一步游戏状态。'],
    'controls.redo': ['Redo', '重做'],
    'controls.redoDescription': ['Reapplies the last undone game step.', '重新执行最近撤销的一步。'],
    'controls.restart': ['Restart game', '重新开始游戏'],
    'controls.restartDescription': ['Uses the existing two-step restart confirmation.', '使用现有的两步确认流程重新开始。'],
    'controls.pressAgainToRestart': ['Press the restart shortcut again to confirm.', '再次按下重新开始快捷键以确认。'],
    'controls.moveRight': ['Move right', '向右移动'],
    'controls.moveDown': ['Move down', '向下移动'],
    'controls.moveLeft': ['Move left', '向左移动'],
    'controls.moveUp': ['Move up', '向上移动'],
    'controls.moveEast': ['Move east', '向东移动'],
    'controls.moveSoutheast': ['Move southeast', '向东南移动'],
    'controls.moveSouthwest': ['Move southwest', '向西南移动'],
    'controls.moveWest': ['Move west', '向西移动'],
    'controls.moveNorthwest': ['Move northwest', '向西北移动'],
    'controls.moveNortheast': ['Move northeast', '向东北移动'],
    'controls.directionDescription': ['Moves in this direction when the current game accepts directional input.', '当前游戏可接收方向输入时，向此方向移动。'],
    'controls.hexHorizontalDescription': ['Horizontal arrow key; diagonal arrow chords remain available.', '使用水平方向键；对角方向键组合仍然可用。'],
    'controls.hexSoutheastDescription': ['Existing chord: Arrow down + Arrow right. Add an alternate key here.', '现有组合：下方向键 + 右方向键；可在此添加备选键。'],
    'controls.hexSouthwestDescription': ['Existing chord: Arrow down + Arrow left. Add an alternate key here.', '现有组合：下方向键 + 左方向键；可在此添加备选键。'],
    'controls.hexNorthwestDescription': ['Existing chord: Arrow up + Arrow left. Add an alternate key here.', '现有组合：上方向键 + 左方向键；可在此添加备选键。'],
    'controls.hexNortheastDescription': ['Existing chord: Arrow up + Arrow right. Add an alternate key here.', '现有组合：上方向键 + 右方向键；可在此添加备选键。'],
    'controls.pointerClick': ['Primary click', '主键点击'],
    'controls.pointerClickDescription': ['Selects or plays the object under the pointer.', '选择或操作指针下方的对象。'],
    'controls.pointerDrag': ['Drag', '拖动'],
    'controls.pointerDragDescription': ['Moves supported pieces, balls, or board controls.', '移动支持拖动的棋子、球或棋盘控件。'],
    'controls.pointerSwipe': ['Swipe', '滑动'],
    'controls.pointerSwipeDescription': ['Moves in directional games on touch screens.', '在触屏上的方向类游戏中移动。'],
    'controls.pointerTouchPan': ['Two-finger drag', '双指拖动'],
    'controls.pointerTouchPanDescription': ['Pan the universal-cover view with two fingers. One finger keeps its game action.', '双指拖动可平移通用覆盖视图。单指仍用于原有游戏操作。'],
    'controls.pointerTouchPinch': ['Pinch', '双指缩放'],
    'controls.pointerTouchPinchDescription': ['Pinch with two fingers to zoom around their midpoint.', '双指捏合或张开，以两指中点为中心缩放视图。'],
    'controls.pointerMiddleDrag': ['Middle drag', '中键拖拽'],
    'controls.pointerMiddleDragDescription': ['Pans the universal-cover view.', '平移通用覆盖视图。'],
    'controls.pointerWheel': ['Mouse wheel', '鼠标滚轮'],
    'controls.pointerWheelDescription': ['Zooms at the pointer.', '以指针位置为中心缩放。'],
    'controls.pointerTrackpad': ['Trackpad', '触控板'],
    'controls.pointerTrackpadDescription': ['Two-finger scroll pans; pinch zooms.', '双指滚动平移；捏合缩放。'],
    'controls.previewProfile': ['game preview', '游戏预览'],
    'controls.profileValue': ['{{game}} · {{lattice}}', '{{game}} · {{lattice}}'],
    'controls.squareGrid': ['square grid', '方格'],
    'controls.hexGrid': ['hex grid', '六角格'],
    ...
    'access.soundVariants': ['Sound effect variant controls', '音效变体控件'],
    'access.soundAudition': ['Seeded retro sound audition', '种子复古音效试听'],
    'access.soundAuditionCategory': ['Sound collection', '音色集合'],
    'access.soundAuditionSeed': ['Sound seed', '音效种子'],
    'access.soundAuditionEvent': ['Sound event', '音效事件'],
    'access.soundAuditionCustomSeed': ['Custom numeric sound seed', '自定义数字音效种子'],
    ...
    'access.fullscreenGameTools': ['Fullscreen game tools', '全屏游戏工具'],
    'access.openFullscreenSettings': ['Open fullscreen settings', '打开全屏设置'],
    'access.closeFullscreenSettings': ['Close fullscreen settings', '关闭全屏设置'],
    'access.soundVolume': ['Sound volume', '音效音量'],
    ...
    'access.confirmRestartGame': ['Confirm restart game', '确认重新开始游戏'],
    ...
    'access.keepNumbersUpright': ['Keep numbers upright', '保持数字正向'],
    ...
    'access.boardView': ['Board view (usual or wrapped)', '棋盘视图（普通或环绕）'],
    'access.tileStyle': ['Tile style', '格子样式'],
    'access.glueFlaps': ['Show glue flaps', '显示粘合片'],
    'access.glueFlapTargetOnHover': ['Show glue target only on hover', '仅悬停时显示粘合目标'],
    ...
    'access.placementHintHighlight': ['Directional hint highlight', '方向提示高亮方式'],
    'access.placementHintColors': ['Directional hint line colors', '方向提示线颜色'],
    ...
    'access.previewOpacity': ['Placement preview opacity', '落子预览透明度'],
    'access.hexCoverOffsetX': ['Hex cover horizontal offset', '六边形覆盖水平偏移'],
    'access.hexCoverOffsetY': ['Hex cover vertical offset', '六边形覆盖垂直偏移'],
    'access.hexCoverOffsetXValue': ['Hex cover horizontal offset value', '六边形覆盖水平偏移值'],
    'access.hexCoverOffsetYValue': ['Hex cover vertical offset value', '六边形覆盖垂直偏移值'],
    ...
    'setup.boardView': ['Board view', '棋盘视图'],
    'setup.tileStyle': ['Tile style', '格子样式'],
    'setup.gluedBoundary': ['Glued boundary', '粘合边界'],
    'setup.glueFlaps': ['Glue flaps', '粘合片'],
    'setup.glueFlapTargetOnHover': ['On hover', '仅悬停'],
    'wrapped.usual': ['usual', '普通'],
    'wrapped.wrapped': ['wrapped', '平铺覆盖'],
    'setup.coordinates': ['coordinates', '坐标'],
    'setup.hintHighlight': ['Hint highlight', '提示高亮'],
    'setup.hintHighlightSingle': ['one direction', '一个方向'],
    'setup.hintHighlightOpposites': ['direction and opposite', '该方向及其反方向'],
    'setup.hintLineColors': ['Hint colors', '提示线颜色'],
    'setup.hintLineColorsUniform': ['one color', '一种颜色'],
    'setup.hintLineColorsAxisCount': ['{{count}} colors — one per axis', '{{count}} 种颜色——每条轴线一种'],
    'setup.hintLineColorsDirectionCount': ['{{count}} colors — one per direction', '{{count}} 种颜色——每个方向一种'],
    ...
    'setup.keepNumbersUpright': ['Keep numbers upright', '保持数字正向'],
    ...
    'setup.previewOpacity': ['Preview opacity', '预览透明度'],
    ...
    'online.waitingTitle': ['Online game waiting', '在线对局等待中'],
    'online.readyPlayerCountOne': ['1 player prepared', '1 名玩家已准备'],
    'online.readyPlayerCountMany': ['{{count}} players prepared', '{{count}} 名玩家已准备'],
    'online.waitingReadyRule': ['Every current player must be prepared before the game begins.', '当前所有玩家准备后，游戏才会开始。'],
    ...
    'setup.tileFamily': ['Characters', '字符'],
    'setup.level': ['Level', '等级'],
    'setup.chineseCharacters': ['Chinese characters', '汉字'],
    'setup.japaneseCharacters': ['Japanese characters', '日文假名'],
    'setup.koreanCharacters': ['Korean characters', '韩文'],
    'setup.ipaCharacters': ['IPA symbols', '国际音标符号'],
    'setup.ipaVowels': ['IPA vowels', '国际音标元音'],
    'setup.ipaPulmonicConsonants': ['IPA pulmonic consonants', '国际音标肺部气流辅音'],
    'setup.ipaNonPulmonicConsonants': ['IPA non-pulmonic consonants', '国际音标非肺部气流辅音'],
    'setup.ipaOtherSymbols': ['IPA other symbols', '国际音标其他符号'],
    'setup.ipaAffricates': ['IPA affricates', '国际音标塞擦音'],
    'setup.ipaAudio': ['IPA audio', '国际音标语音'],
    'setup.ipaAudioSource': ['IPA Chart audio', 'IPA Chart 语音'],
    ...
    'setup.koreanJamoVowels': ['Korean jamo: basic vowels', '韩文字母：基础元音'],
    'setup.koreanJamoBasic': ['Korean jamo: basic combined', '韩文字母：基础合并'],
    'setup.koreanJamoModern': ['Korean jamo: modern extensions', '韩文字母：现代扩展'],
    'setup.koreanJamoCombined': ['Korean jamo: final exam', '韩文字母：综合测验'],
    'setup.koreanSpellingA': ['Korean spelling 1: ㅏ syllables', '韩文拼写 1：ㅏ 音节'],
    'setup.koreanSpellingVowels': ['Korean spelling 2: vowel sounds', '韩文拼写 2：元音发音'],
    'setup.koreanSpellingCv': ['Korean spelling 3: basic CV', '韩文拼写 3：基础辅音－元音'],
