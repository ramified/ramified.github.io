...
export function createEditorContext(host,definition,{id,onChange=()=>{},onError=console.error,snapshot=null}={}){
  ...
  let active=true,disposed=false,ready=false,bridge=null,stateBinding=null,handlers={},link={},saving=false,dock=null;
  ...
  const mathApi=realWindow.MathJax?.typesetPromise?new Proxy(realWindow.MathJax,{get(target,key){
    if(key==='typesetPromise')return (elements=[body])=>{
      const run=mathQueue.then(()=>{if(!disposed)return target.typesetPromise(elements);}).then(()=>{
        const css=realDocument.getElementById('MJX-SVG-styles');if(css&&!shadow.getElementById('MJX-SVG-styles'))head.append(css.cloneNode(true));
  ...
  function listen(target,type,fn,opts){
    ...
    const callback=e=>{
      if(disposed||(!active&&['keydown','keyup','pointerdown','pointermove','pointerup','mousedown','mouseup','mousemove','wheel','resize'].includes(type)))return;
      if(['keydown','keyup'].includes(type)&&!shadow.contains(realDocument.activeElement)&&realDocument.activeElement!==host)return;
      if(['pointerdown','mousedown','click'].includes(type)&&!e.composedPath().includes(host))return;
  ...
  const raf=fn=>{const callback=t=>{try{fn(t);}catch(e){error(e);}},token=++sequence,item={fn:callback,native:null};rafs.set(token,item);if(active)item.native=realWindow.requestAnimationFrame(t=>{rafs.delete(token);if(!disposed)callback(t);});return token;};
  ...
  const facadeDocument=new Proxy(Object.create(Object.getPrototypeOf(realDocument)),{get(target,key){
    ...
    if(key==='contains')return node=>shadow.contains(node);
    if(key==='defaultView')return environment.window;
    if(key==='getElementsByClassName')return (...args)=>docRoot.getElementsByClassName(...args);
    if(key==='getElementsByTagName')return (...args)=>docRoot.getElementsByTagName(...args);
    if(key==='getElementById')return k=>shadow.getElementById(k);
    if(key==='querySelector')return s=>s==='body'?body:s==='html'?docRoot:shadow.querySelector(s);
    if(key==='querySelectorAll')return s=>shadow.querySelectorAll(s);
  ...
  function bindInline(){
    for(const node of shadow.querySelectorAll('*'))for(const a of [...node.attributes])if(/^on[a-z]+$/.test(a.name)){
  ...
  const inlineObserver=new realWindow.MutationObserver(bindInline);inlineObserver.observe(body,{subtree:true,childList:true});disposal.push(()=>inlineObserver.disconnect());
  for(const type of ['input','change','click','pointerup','keyup'])body.addEventListener(type,changed);
  function uiSnapshot(){return {controls:[...body.querySelectorAll('input[id],select[id],textarea[id]')].filter(n=>n.type!=='file').map(n=>({id:n.id,value:n.value,checked:n.checked})),cards:[...body.querySelectorAll('.card')].map((n,i)=>({i,key:n.dataset.workspaceCardId,collapsed:n.classList.contains('collapsed'),hidden:n.hidden,userHidden:n.classList.contains('calculator-card-user-hidden'),pinned:n.classList.contains('is-pinned'),wide:n.dataset.cardWideState})),dock:dock?.capture(),scrollTop:host.scrollTop};}
  function restoreUi(ui){if(!ui)return;for(const c of ui.controls||[]){const n=shadow.getElementById(c.id);if(n&&n.type!=='file'){n.value=c.value;if(c.checked!==undefined)n.checked=c.checked;}}for(const c of ui.cards||[]){const list=[...body.querySelectorAll('.card')],n=c.key?list.find(n=>n.dataset.workspaceCardId===c.key):list[c.i];if(n){n.classList.toggle('collapsed',c.collapsed);n.hidden=!!c.hidden;n.classList.toggle('calculator-card-user-hidden',!!c.userHidden);n.classList.toggle('is-pinned',!!c.pinned);n.querySelector('.card-head')?.setAttribute('aria-expanded',String(!c.collapsed));if(c.wide&&win.CalculatorCards)win.CalculatorCards.setWide(n,c.wide==='wide');}}dock?.restore(ui.dock);host.scrollTop=ui.scrollTop||0;}
  ...
  const api={host,shadow,environment,applyState,linkGlobals:v=>{link=v;},registerHandlers:v=>{handlers=v;},registerState:v=>{stateBinding=v;},setBridge:v=>{bridge=v;},capture,restore,
    activate(){if(disposed)return;active=true;host.hidden=false;dock?.update();for(const [token,item] of rafs)if(item.native===null)item.native=realWindow.requestAnimationFrame(t=>{rafs.delete(token);if(!disposed)item.fn(t);});bridge?.resize?.();for(const l of listeners.filter(l=>l.target===realWindow&&l.type==='resize'))l.callback(new Event('resize'));},
    deactivate(){bridge?.deactivate?.();for(const l of listeners.filter(l=>l.target===realWindow&&l.type==='blur'))l.callback(new Event('blur'));active=false;for(const item of rafs.values()){if(item.native!==null)realWindow.cancelAnimationFrame(item.native);item.native=null;}host.hidden=true;},
    dispose(){if(disposed)return;api.deactivate();disposed=true;mathApi?.typesetClear();for(const l of listeners)l.target.removeEventListener(l.type,l.callback,l.opts);timers.forEach(realWindow.clearTimeout);intervals.forEach(realWindow.clearInterval);workerSet.forEach(w=>w.terminate());disposal.forEach(f=>f());host.remove();},
    start(){bindInline();ready=true;for(const fn of readyListeners){try{fn.call(facadeDocument,new Event('DOMContentLoaded'));}catch(e){error(e);}}for(const fn of loadListeners){try{fn.call(win,new Event('load'));}catch(e){error(e);}}bindInline();dock=createCardDock(body,{changed,resize:()=>bridge?.resize?.()});if(snapshot)restore(snapshot);mathApi?.typesetPromise().catch(error);return api;}
  };
  return api;
}
